...
class ConfigTests(unittest.TestCase):
    def test_repository_config_is_valid(self) -> None:
        ...
        self.assertEqual(config["pipeline"]["http"]["maximum_attempt_ttfb_ms"], 300)
        self.assertTrue(config["pipeline"]["tcp"]["require_all_attempts"])
        self.assertEqual(config["pipeline"]["tcp"]["maximum_attempt_latency_ms"], 300)
        self.assertEqual(config["pipeline"]["current_selection"], 500)
        self.assertEqual(config["pipeline"]["speed"]["minimum_mbps"], 16.0)
