...
class ConfigTests(unittest.TestCase):
    def test_repository_config_is_valid(self) -> None:
        ...
        self.assertEqual(config["pipeline"]["local_probe_batch_size"], 100)
        ...
        self.assertEqual(config["pipeline"]["quality_tcp"]["attempts"], 3)
        ...
        self.assertEqual(config["pipeline"]["speed"]["bytes_per_test"], 524288)
        self.assertEqual(config["pipeline"]["speed"]["concurrency"], 20)
        self.assertEqual(config["_local_rules"]["latency_max_ms"], 200)
        self.assertFalse(config["_local_options"]["continuous_three_rounds"])
        self.assertEqual(config["pipeline"]["http"]["attempts"], 3)
        ...
        self.assertEqual(config["handoff"]["target"], 10000)
        self.assertEqual(config["handoff"]["max_replenishment_rounds"], 3)
