...
class ConfigTests(unittest.TestCase):
    def test_repository_config_is_valid(self) -> None:
        ...
        self.assertEqual(config["pipeline"]["strict_tcp_candidates_per_round"], 10000)
        self.assertEqual(config["pipeline"]["rolling_candidate_batch"], 800)
        self.assertEqual(config["pipeline"]["speed_batch_size"], 1000)
        self.assertEqual(config["pipeline"]["tcp"]["attempts"], 3)
        ...
        self.assertEqual(config["pipeline"]["tcp"]["maximum_average_latency_ms"], 200)
        ...
        self.assertEqual(config["pipeline"]["country_minimums"], {"JP": 15})
        self.assertEqual(config["pipeline"]["rolling_retest"]["attempts"], 3)
