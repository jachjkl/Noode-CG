...
class ScorerTests(unittest.TestCase):
    ...
    def test_reserves_ten_japanese_nodes_without_a_korean_quota(self) -> None:
        ranked = [
            node(f"104.16.{index // 250}.{index % 250 + 1}", latency=index + 1, speed=80, country="US")
            for index in range(30)
        ]
        ranked.extend(
            node(f"172.64.{index}.1", latency=200 + index, speed=20, country="JP")
        ...
        countries = [item.country for item in selected]
        self.assertEqual(len(selected), 30)
        self.assertGreaterEqual(countries.count("JP"), 10)
