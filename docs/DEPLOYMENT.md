...
- 服务账号可以使用默认的 `NT AUTHORITY\NETWORK SERVICE`；V13.6.2 安装器会只对 `D:\桌面\软件\Noode-CG-Local` 授予所需权限；
...
如果本地步骤一直显示 queued，检查 Runner 服务是否正在运行以及标签是否包含 `noode-cg`。如果提示 D 盘应用版本不是 13.6.2，请在解压后的 V13.6.2 `Noode-CG` 目录重新运行 `scripts\install-windows-controller.ps1`。如果直接网络检查失败，关闭代理后在 Actions 页面重新运行。
